...
contract Upload{
    ...
    // revieww the image - by Scientist 
    function review_image(address _user,string memory _url,string memory _solution) public{
     ...
     //   delete(closeList[_url]);  // why this i didnt get it
        closeImg[_user].push(_url);
        final_images.push(_url);
    }

    // verify the images - by all other  // how to change the count of the verification , etc etc
    function verify_image(address _user, string memory _url) public{
     ...
     Plant_Image storage new_image = closeList[_url];
        ...
        require(closeList[_url].owner != msg.sender,"you are not allowed to verify your own image");
           ...
           string[] memory strings = finalImg[_user];
       bool flag2=false;
        ...
        if(flag2==false){
            ...
            finalImg[_user].push(_url);  
        }
        
        farmer_map[msg.sender].image_VR.push(_url);
        scientist_map[msg.sender].image_VR.push(_url);
    }
    ...
    // to get pending image 
    function display_pending(address _user) public view returns(string[] memory){
        return PendingImg[_user];
    }
    ...
    // Function to get the level of a scientist
    function getScientistLevel(address _scientist) public view returns (uint256) {
        return scientist_map[_scientist].level;
    }

    // Function to get the level of a farmer
    function getFarmerLevel(address _farmer) public view returns (uint256) {
        return farmer_map[_farmer].level;
    }

    // get the ai-solution
    function get_ai_sol(string memory _url) public view returns(string memory){
        Plant_Image storage new_image = finalList[_url];
        return new_image.AI_sol;
    }
    // get thescientist sol
    function get_scientist_sol(string memory _url) public view returns(string memory){
        Plant_Image storage new_image = finalList[_url];
        return new_image.reviewer_sol;
    }

    // get reviewer address 
    function get_reviewer_add(string memory _url) public view returns(address){
        Plant_Image storage new_image = finalList[_url];
        return new_image.reviewer;
    }
